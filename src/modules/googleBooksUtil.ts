  ...
  try{
    ...
    if(response && response.data && response.data.totalItems > 0){
      ...
      books = response.data.items.map((bookItem:GoogleBookItem) => {
        ...
        const authorName = volumeInfo.authors && volumeInfo.authors.length > 0 ? volumeInfo.authors.join(",") : null;
        // 書影は小サムネ＞大サムネ＞空の順で設定する
        const coverUrl = volumeInfo.imageLinks ? (volumeInfo.imageLinks.smallThumbnail || volumeInfo.imageLinks.thumbnail) : null;
