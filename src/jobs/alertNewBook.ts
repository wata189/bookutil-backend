...
const discoverNewBook = async (fs: firestoreUtil.FirestoreTransaction) => {
  ...
  const events = Object.keys(calender)
    .map((key) => calender[key])
    .filter((cal) => {
      let startStr = "00000000";
      if ("start" in cal && cal.start) {
        if ("start" in cal.start) {
          startStr = util.formatDateToStr(
            cal.start.start,
            onlyNumberdateFormat
          );
        } else {
...
`;
  ...
  for (const newBook of newBooks) {
    // DB格納
    promises.push(
      fs.createDocument(firestoreUtil.COLLECTION_PATH.T_NEW_BOOK, newBook)
    );
...
const main = async () => {
  ...
  await firestoreUtil.tran([discoverNewBook]);
